...
- Items price editing plus item metadata/behavior guidance, text/dialogue editing, trainer basics, wild encounter basics, shops inventory basics, raid rewards basics, placement item basics, spreadsheet import, flagwork/save inspection, ExeFS patch readiness, and Royal Candy workflow guidance.
...
| Items | Prices, pouch/inventory metadata, field-use behavior, TM/TR machine data, battle boosts, Pokemon effects, raw flags, trade-evolution item bulk action | Price editing plus backend-decoded inventory, field-use, battle, and Pokemon-effect detail groups in the inspector/search | Add guarded item metadata editing, TM/TR taught-move linkage, item selector filters, and previewed trade-evolution item transforms |
...
4. Expanded Items workflow beyond prices now includes backend-decoded field-use behavior, Pokemon effects, battle boosts, and TM/TR machine metadata guidance; continue with guarded item metadata editing, TM/TR taught-move linkage, and previewed item transforms.
