...
- Pokemon Data coverage for personal records, editable Personal scalar/flag fields, TM/TR/tutor compatibility flags, evolution records, learnsets, names, search, provenance, diagnostics, and output-root apply.
...
KM now presents core Personal scalar/flag fields as backend-owned edit-session fields with validation, diagnostics, change previews, output-root writes, named selectors for type, egg group, EXP growth, and color fields, and grouped TM/TR/type-tutor/Armor-tutor compatibility toggles. Future Pokemon Data slices should add item/ability/species reference labels, level-up learnset row editing, evolution row editing, and previewed transforms that list every affected species and output file before apply.
...
1. Pokemon Data editable parity foundation: personal fields and TM/TR/tutor compatibility flags are started with guarded edit sessions and core enum selectors; continue with item/ability/species selectors, learnset row edits, evolution row edits, and previewed bulk transforms.
