...
| Shops | Single-shop and multi-shop inventories, hash labels, rotating/multiple inventories, item-name formatting | Basic inventory editing with backend-provided item selector metadata, item names, prices, and source provenance | Improve single/multi labels, inventory grouping, search, and linked item validation |
...
7. Shops and placement expansion for multi-inventory shops, static spawns, trainer refs, nest refs, richer hash labels, and deeper linked item validation.
