...
- Items price editing plus guarded item metadata/behavior editing, text/dialogue editing, trainer basics, wild encounter basics, shops inventory basics, raid rewards basics, placement item basics, spreadsheet import, flagwork/save inspection, ExeFS patch readiness, and Royal Candy workflow guidance.
...
| Items | Prices, pouch/inventory metadata, field-use behavior, TM/TR machine data, battle boosts, Pokemon effects, raw flags, trade-evolution item bulk action | Price editing plus guarded editing for stable pouch/inventory, field-use, named cure/use/status flags, named battle boost fields, EV/heal/PP/friendship, and effect metadata fields; TM/TR taught-move linkage and editing; backend-decoded inventory, field-use, battle, and Pokemon-effect detail groups stay visible in the inspector/search | Add item selector filters and previewed trade-evolution item transforms |
| Shops | Single-shop and multi-shop inventories, hash labels, rotating/multiple inventories, item-name formatting | Inventory editing with backend-provided item selector metadata, item names/prices, source provenance, single/multi kind labels, inventory labels, source hashes, and inventory summaries | Add deeper linked item validation/navigation and advanced multi-inventory operations |
...
4. Expanded Items workflow beyond prices now includes backend-decoded field-use behavior, Pokemon effects, battle boosts, TM/TR machine metadata guidance, guarded metadata editing for stable item row fields, TM/TR taught-move editing, and named item behavior flag/boost controls; continue with item filters and previewed item transforms.
...
7. Shops and placement expansion for deeper linked item validation/navigation, advanced multi-inventory operations, static spawns, trainer refs, nest refs, and richer hash labels.
