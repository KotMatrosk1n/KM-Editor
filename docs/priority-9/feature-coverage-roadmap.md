...
- Pokemon Data coverage for personal records, editable Personal scalar/flag fields, TM/TR/tutor compatibility flags, editable level-up learnset rows, evolution records, names, search, provenance, diagnostics, and output-root apply.
...
KM now presents core Personal scalar/flag fields as backend-owned edit-session fields with validation, diagnostics, change previews, output-root writes, named selectors for type, egg group, EXP growth, and color fields, grouped TM/TR/type-tutor/Armor-tutor compatibility toggles, and level-up learnset row add/edit/remove/reorder actions against `wazaoboe_total.bin`. Future Pokemon Data slices should add item/ability/species reference labels, evolution row editing, and previewed transforms that list every affected species and output file before apply.
...
1. Pokemon Data editable parity foundation: personal fields, TM/TR/tutor compatibility flags, core enum selectors, and level-up learnset row editing are covered with guarded edit sessions; continue with item/ability/species selectors, evolution row edits, and previewed bulk transforms.
