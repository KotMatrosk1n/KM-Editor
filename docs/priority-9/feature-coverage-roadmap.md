...
| Items | Prices, pouch/inventory metadata, field-use behavior, TM/TR machine data, battle boosts, Pokemon effects, raw flags, trade-evolution item bulk action | Price editing plus guarded editing for stable pouch/inventory, field-use, cure/use-flag, EV/heal/PP/friendship, and effect metadata fields; TM/TR taught-move linkage and editing; backend-decoded inventory, field-use, battle, and Pokemon-effect detail groups stay visible in the inspector/search | Add safer named flag/boost toggles, item selector filters, and previewed trade-evolution item transforms |
