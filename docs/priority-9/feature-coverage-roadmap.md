...
- Gift Pokemon, Trade Pokemon, Static Encounters, and Rental Pokemon coverage for scripted individual Pokemon records, named selectors, EV/IV controls, sentinel/preset or fixed-IV validation, search, provenance, diagnostics, and output-root apply.
- Raid Battles coverage for regular nest battle slots, species/form, star probabilities, ability roll, gender, Gigantamax state, guaranteed perfect IV count, linked table hashes, search, provenance, diagnostics, and output-root apply.
...
KM should implement a shared individual-Pokemon stats model for IVs, EVs, nature, gender, ability, shiny, Dynamax, Gigantamax, and moves, while keeping each workflow's binary layout and sentinel validation in the backend. Trainer parties, gifts, trades, static encounters, rentals, and regular raid battle records now cover IV-related editing through guarded backend edit sessions; Dynamax Adventures remain in the queue.
...
2. Individual Pokemon stat editing across trainer parties first, then gifts/trades/statics/rentals/Dynamax Adventures, with IV handling treated as a required shared capability. Trainer-party, gift, trade, static encounter, rental, and regular raid battle IV/stat editing are covered; continue with Dynamax Adventures, raid placement usage labels, linked reward navigation, stronger trainer labels, and safe bulk helpers.
...
5. Gift, trade, static, rental, and Dynamax Adventure encounter workflows, including linked text/placement side effects. Gift, trade, static encounter, and rental editing are covered for stable fields; continue with Dynamax Adventures and linked side-effect modeling.
6. Raid battle workflow coverage is in place for regular nest battle slots; continue with stronger raid reward/bonus reward linking, usage labels, placement provenance, and remaining encounter variants.
