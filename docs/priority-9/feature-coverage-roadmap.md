...
| Items | Prices, pouch/inventory metadata, field-use behavior, TM/TR machine data, battle boosts, Pokemon effects, raw flags, trade-evolution item bulk action | Price editing plus guarded editing for stable pouch/inventory, field-use, cure/use-flag, EV/heal/PP/friendship, and effect metadata fields; backend-decoded inventory, field-use, battle, and Pokemon-effect detail groups stay visible in the inspector/search | Add TM/TR taught-move linkage, safer named flag/boost toggles, item selector filters, and previewed trade-evolution item transforms |
...
4. Expanded Items workflow beyond prices now includes backend-decoded field-use behavior, Pokemon effects, battle boosts, TM/TR machine metadata guidance, and guarded metadata editing for stable item row fields; continue with TM/TR taught-move linkage, named flag/boost toggles, and previewed item transforms.
