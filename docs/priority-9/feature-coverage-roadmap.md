...
KM should implement a shared individual-Pokemon stats model for IVs, EVs, nature, gender, ability, shiny, Dynamax, Gigantamax, and moves, while keeping each workflow's binary layout and sentinel validation in the backend. Trainer parties, gifts, trades, static encounters, and rentals now cover IV editing through guarded backend edit sessions; Dynamax Adventures and raid battle records remain in the queue.
