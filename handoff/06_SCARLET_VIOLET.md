...
- Pokemon Data display names are backend-owned strings. Do not run Pokemon Data `record.name` values back through the shared species/form formatter in the desktop. That formatter is for surfaces that receive plain species names plus raw form values, such as encounters and trainer slots. SwSh Pokemon Data and S/V Pokemon Data can use different form numbering semantics from those other surfaces, so reformatting already-resolved Pokemon Data names can reintroduce duplicate suffixes or bogus labels such as `Form 1`.
...
S/V Hyperspace Hoopa move restriction research on 2026-06-19:

* Hyperspace Hole is move `593` (`0x251`) and Hyperspace Fury is move `621` (`0x26D`) in `avalon/data/waza_array.bin`.
* Both moves are globally enabled in move data, share effect sequence `359`, share `affinity=5`, and have no unknown move flags `56..60` or unused flags `61..70` set. Effect sequence `359` is unique to these two moves in the current Scarlet/Violet move table and appears to identify shared Hyperspace behavior, not the Hoopa form gate.
* Normal learnability is split in `avalon/data/personal_array.bin`: Hoopa form `0` row index `1019` has Hyperspace Hole in TM moves and level-up at `85`; Hoopa form `1` row index `1020` has Hyperspace Fury in TM moves and level-up at `85`.
* Static EXEFS scan found the relevant Hoopa/form code in both Scarlet build `421C5411B487EB4D049DD065FEC9547773E8E598` and Violet build `709BFD66115298640155FCC4979DBA151C7CC79A` at matching `.text` offsets. `.text+0x01FE76A0` maps species/form to a signature move: at `.text+0x01FE772C` it checks species `0x2D0` and chooses form `0 -> 0x251` or form `1 -> 0x26D`. `.text+0x01FE79B4` scans move slots for form-specific signature moves; the Hoopa block starts at `.text+0x01FE7D40`, checks species `0x2D0`, then checks slots `0..3` for `0x251` and `0x26D`.
* A smaller `.text+0x002BD4B8` cluster references `0x26D` and species `0x2D0`, but local disassembly suggests it is a parameterized routine/assert/log path, not the Hoopa form rule. The tight cluster containing species plus both Hyperspace move IDs is `.text+0x01FE7718..0x01FE772C` and `.text+0x01FE7D40..0x01FE7E1C`.
