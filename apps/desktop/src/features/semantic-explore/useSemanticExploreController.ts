...
class SemanticExploreControllerStore {
  ...
  private detachedCancellationGeneration = 0;
  ...
  private slotGenerations = new Map<SlotName, number>();
  ...
  public refreshCapabilities() {
    ...
    const pending = this.readCapabilities();
    this.capabilitiesPromise = pending;
    void pending.finally(() => {
      if (this.capabilitiesPromise === pending) {
  ...
  private async readCapabilities() {
    ...
    try {
      const response = await this.bridge.getCapabilities({ scope });
  ...
  public async searchEntityCommands(searchText: string, limit: number) {
    ...
    const expectedCancellationGeneration = this.detachedCancellationGeneration;
    if (!this.snapshot.capabilities.data) {
      await this.ensureCapabilities();
    }
    if (
      expectedEpoch !== this.epoch ||
      expectedCancellationGeneration !== this.detachedCancellationGeneration
    ) {
  ...
  public cancelEntityCommandSearch() {
    this.detachedCancellationGeneration += 1;
  ...
  public async getEntity(record: SemanticExploreRecordRef, layer: QueryableLayer) {
    ...
    try {
      ...
      const response = await this.bridge.getEntity({
  ...
  private async runComparison(append: boolean) {
    ...
    try {
      const context = this.requireQueryContext();
      const response = await this.bridge.compare({
        ...(cursor ? { cursor } : {}),
        expectedRevision: context.capabilities.revision,
        left: spec.left,
        limit: semanticExploreDefaultPageSize,
      ...
      this.completePage(token, 'comparison', response, previous, append, mergeItemPages);
    } catch (error) {
  ...
  private async runReferences(append: boolean) {
    ...
    try {
      const context = this.requireQueryContext();
      const response = await this.bridge.getReferences({
        ...(cursor ? { cursor } : {}),
      ...
      this.completePage(token, 'references', response, previous, append, mergeItemPages);
    } catch (error) {
  ...
  private async runImpact(append: boolean) {
    ...
    try {
      const context = this.requireQueryContext();
      const response = await this.bridge.getImpact({
        ...(cursor ? { cursor } : {}),
        expectedRevision: context.capabilities.revision,
        layer: spec.layer,
        limit: semanticExploreDefaultPageSize,
      ...
      this.completePage(token, 'impact', response, previous, append, mergeItemPages);
    } catch (error) {
  ...
  private async runOwnership(append: boolean) {
    ...
    try {
      const context = this.requireQueryContext();
      const response = await this.bridge.getOwnership({
        ...(cursor ? { cursor } : {}),
        expectedRevision: context.capabilities.revision,
        limit: semanticExploreDefaultPageSize,
        ...(spec.record ? { record: spec.record } : {}),
      ...
      this.completePage(token, 'ownership', response, previous, append, mergeOwnershipPages);
    } catch (error) {
  ...
  private async runInitialExternalComparison(externalRootPath: string) {
    ...
    try {
      const context = this.requireQueryContext();
      const responsePromise = this.bridge.compareExternal({
        expectedRevision: context.capabilities.revision,
        externalRootPath,
        left: spec.left,
        limit: semanticExploreDefaultPageSize,
  ...
  private async runExternalComparison(append: boolean) {
    ...
    try {
      const context = this.requireQueryContext();
      const response = await this.bridge.compareExternal({
  ...
  private async runChanges(append: boolean) {
    ...
    try {
      const context = this.requireQueryContext();
      const response = await this.bridge.getSemanticChanges({
        ...(cursor ? { cursor } : {}),
        expectedRevision: context.capabilities.revision,
        format: spec.format,
        from: spec.from,
  ...
  private begin(slot: SlotName, append: boolean): RequestToken {
    const slotGeneration = (this.slotGenerations.get(slot) ?? 0) + 1;
    this.slotGenerations.set(slot, slotGeneration);
    const token = {
      epoch: this.epoch,
  ...
  private startDetachedRequest(): DetachedRequestToken {
    const token = {
      cancellationGeneration: this.detachedCancellationGeneration,
      epoch: this.epoch,
      id: this.nextRequestId++
    };
    this.activeRequests.add(token.id);
  ...
  private isDetachedCurrent(token: DetachedRequestToken) {
    return token.epoch === this.epoch &&
      token.cancellationGeneration === this.detachedCancellationGeneration;
  }
  ...
  private isCurrent(token: RequestToken) {
    return token.epoch === this.epoch &&
      this.slotGenerations.get(token.slot) === token.slotGeneration;
  }
  ...
  private clearSlot(slot: SlotName) {
    this.slotGenerations.set(slot, (this.slotGenerations.get(slot) ?? 0) + 1);
