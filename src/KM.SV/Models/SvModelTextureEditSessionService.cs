...
public sealed class SvModelTextureEditSessionService
{
    public EditSession Stage(ProjectPaths paths, EditSession? session, ModelTextureIntent intent)
    {
        ...
        var resource = new SvModelPreviewService().Textures(project, intent.Model).SingleOrDefault(t => t.Id == intent.Texture)
        ...
        if (diagnostics.Count == 0)
        {
            try
            {
                ...
                foreach (var (path, bytes) in outputs)
                {
                    ...
                    if (!readback.AsSpan().SequenceEqual(bytes)) throw new IOException("Texture output did not match its verified encoding.");
                    _ = new ModelTextureDocument(readback);
    ...
    private static Dictionary<string, byte[]> Outputs(ProjectPaths paths, EditSession session)
    {
        ...
        foreach (var edit in session.PendingEdits)
        {
            ...
            if (!resources.TryGetValue(intent.Model, out var models)) resources.Add(intent.Model, models = new SvModelPreviewService().Textures(project, intent.Model));
