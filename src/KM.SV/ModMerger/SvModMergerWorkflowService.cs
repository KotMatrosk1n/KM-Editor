...
public sealed class SvModMergerWorkflowService
{
        ...
        IReadOnlyList<SvModMergerSourceRequest> modSources)
    {
        ...
        var ownerId = new OwnershipOwnerId("workflow.sv.mod-merger");
        var desiredVirtualPaths = plan.Outputs
            .Select(output => StripRomFsPrefix(output.RelativePath))
            .ToHashSet(StringComparer.OrdinalIgnoreCase);
        var ownedVirtualPaths = SvWorkflowFileSource.GetOwnedStandaloneRomFsVirtualPaths(
            project.Paths,
            ownerId);
        ...
        {
            preview = CreatePreview(project, plan.Files, diagnostics);
            return new SvModMergerApplyResult(workflow, preview, [], diagnostics);
        }
        ...
        var deletes = ownedVirtualPaths
            ...
            .ToArray();
        var reviewHash = CreateOutputReviewHash(project.Paths, plan.Outputs, deletes);
        ...
        try
        {
            SvWorkflowFileSource.ApplyStandaloneRomFsReplacementBatch(
                project.Paths,
                plan.Outputs
            ...
            diagnostics.Add(CreateDiagnostic(
                ...
                $"Applied {writtenFiles.Count} Scarlet/Violet LayeredFS file{(writtenFiles.Count == 1 ? string.Empty : "s")} from {analysis.EnabledSourceCount} enabled mod source{(analysis.EnabledSourceCount == 1 ? string.Empty : "s")}."));
        }
            ...
            or ArgumentException)
        {
            diagnostics.Add(CreateDiagnostic(
                DiagnosticSeverity.Error,
                $"S/V Mod Merger output could not be committed atomically: {exception.Message}",
                expected: "Current reviewed output targets and recoverable transaction state"));
            writtenFiles.Clear();
        }

        preview = CreatePreview(project, plan.Files, diagnostics);
        return new SvModMergerApplyResult(
            workflow,
            preview,
        ...
        ICollection<ValidationDiagnostic> diagnostics)
    {
        ...
        foreach (var entry in manifest.Entries)
        {
            ...
            diagnostics.Add(CreateDiagnostic(
                DiagnosticSeverity.Error,
                "A previous S/V Mod Merger output predates transactional ownership and cannot be removed automatically.",
                file: entry.RelativePath,
        ...
        IReadOnlyList<string> deletes)
    {
        ...
        AppendReviewText(hash, "km-sv-mod-merger-output-v1");
