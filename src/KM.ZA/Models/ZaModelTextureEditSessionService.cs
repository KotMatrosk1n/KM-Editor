...
public sealed class ZaModelTextureEditSessionService
{
    public EditSession RestoreVanilla(ProjectPaths paths, EditSession? session, string model)
    {
        ...
        var service = new ZaModelPreviewService();
    ...
    private static Dictionary<string, byte[]> Outputs(ProjectPaths paths, EditSession session)
    {
        ...
        foreach (var edit in session.PendingEdits)
        {
            ...
            if (!resources.TryGetValue(intent.Model, out var models)) resources.Add(intent.Model, models = assetModels.Contains(intent.Model) ? new ZaModelPreviewService().Assets(project, intent.Model) : new ZaModelPreviewService().Textures(project, intent.Model));
            ...
            {
                if (!vanilla.TryGetValue(intent.Model, out var bases)) vanilla.Add(intent.Model, bases = new ZaModelPreviewService().Assets(project, intent.Model, true));
