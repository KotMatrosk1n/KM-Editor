...
public static class ZaMovesMergeDocument
{
    public static MergeDocument Read(byte[] bytes) => ZaMovesEditSessionService.ReadMergeDocument(bytes);
}
...
internal sealed partial class ZaMovesEditSessionService
{
    ...
    {
        var table = KM.Formats.ZA.Generated.GameData.ZaMoveDataArray.GetRootAsZaMoveDataArray(new ByteBuffer(bytes));
