...
public sealed class ZaTypeChartEditSessionService
{
    public const string TypeChartEditDomain = "workflow.typeChart";

    private const string ChartRecordId = "za-type-chart";
    private const string UninstallRecordId = "za-type-chart-v1-uninstall";
    ...
    public ZaTypeChartEditSessionService(
        ProjectWorkspaceService? projectWorkspaceService = null,
        ZaTypeChartWorkflowService? typeChartWorkflowService = null)
    {
        this.projectWorkspaceService = projectWorkspaceService ?? new ProjectWorkspaceService();
        this.typeChartWorkflowService = typeChartWorkflowService ?? new ZaTypeChartWorkflowService();
    }

    public ZaTypeChartEditResult StageChart(
        ProjectPaths paths,
        ...
        EditSession? session)
    {
        ...
        if (currentSession.PendingEdits.Any(edit => !string.Equals(edit.Domain, TypeChartEditDomain, StringComparison.Ordinal)))
        {
            ...
            return new ZaTypeChartEditResult(workflow, currentSession, diagnostics);
        }
        ...
        return new ZaTypeChartEditResult(workflow, updatedSession, diagnostics);
    }

    public ZaTypeChartEditResult StageUninstall(ProjectPaths paths, EditSession? session)
    {
        ArgumentNullException.ThrowIfNull(paths);
        ...
        ZaOutputMode outputMode = ZaOutputMode.Standalone)
    {
        ...
        if (outputMode != ZaOutputMode.Standalone)
        {
            diagnostics.Add(CreateDiagnostic(
                DiagnosticSeverity.Error,
                "Type Chart edits exefs/main, which is outside Pokemon Legends Z-A RomFS output modes. Use standalone LayeredFS output for this editor.",
        ...
        var write = new PlannedFileWrite(
            ...
            isUninstall
                ...
                : "Update the Pokemon Legends Z-A type-effectiveness table in exefs/main.");
        ...
        ICollection<ValidationDiagnostic> diagnostics)
    {
        if (project.Paths.SelectedGame is not ProjectGame.ZA)
        {
            diagnostics.Add(CreateDiagnostic(
                DiagnosticSeverity.Error,
                "Type Chart requires a Pokemon Legends Z-A project.",
                expected: "Pokemon Legends Z-A project"));
            return false;
        }

        if (!project.Health.CanOpenEditableWorkflows || workflow.Summary.Availability != ZaWorkflowAvailability.Available)
        {
            diagnostics.Add(CreateDiagnostic(
                DiagnosticSeverity.Error,
        ...
        if (workflow.InstallStatus == "blocked")
        {
            if (!diagnostics.Any(diagnostic => diagnostic.Severity == DiagnosticSeverity.Error))
            {
                diagnostics.Add(CreateDiagnostic(
                    ...
                    expected: "Known Pokemon Legends Z-A exefs/main type chart table"));
            }
        ...
        ICollection<ValidationDiagnostic> diagnostics)
    {
        if (project.Paths.SelectedGame is not ProjectGame.ZA)
        {
            diagnostics.Add(CreateDiagnostic(
                DiagnosticSeverity.Error,
                "Type Chart uninstall requires a Pokemon Legends Z-A project.",
                expected: "Pokemon Legends Z-A project"));
            return false;
        }
        ...
        ICollection<ValidationDiagnostic> diagnostics)
    {
        try
        {
            ZaTypeChartMainPatcher.ValidateValues(values);
            return true;
        }
        catch (InvalidDataException exception)
        {
    ...
    private static int[] DecodeValues(string? value, ICollection<ValidationDiagnostic> diagnostics)
    {
        if (string.IsNullOrWhiteSpace(value))
        {
            ...
            return ZaTypeChartMainPatcher.VanillaChartValues.ToArray();
        }
        ...
        ICollection<ValidationDiagnostic> diagnostics)
    {
        ...
        var targetPath = ZaTypeChartWorkflowService.ResolveOutputPath(paths, ZaTypeChartWorkflowService.ExeFsMainPath);
        if (targetPath is null)
        {
            diagnostics.Add(CreateDiagnostic(
                DiagnosticSeverity.Error,
