...
public sealed class ZaModMergerWorkflowService
{
    private const string WorkflowId = "modMerger";
    private const string WorkflowDomain = "workflow.zaModMerger";
    ...
    public ZaModMergerWorkflowService(ProjectWorkspaceService? projectWorkspaceService = null)
    {
    ...
    public ZaWorkflowSummary CreateSummary(OpenedProject project)
    {
        ...
        return new ZaWorkflowSummary(
            WorkflowId,
            "Mod Merger",
            "Smart merge ordered Pokemon Legends ZA RomFS mods from folders, zip files, or rar files and patch the Trinity descriptor for LayeredFS.",
            availability,
            []);
    }

    public ZaModMergerWorkflow Load(
        ProjectPaths paths,
    ...
    public ZaModMergerStageResult Stage(
        ProjectPaths paths,
        IReadOnlyList<ZaModMergerSourceRequest> modSources)
    {
        ArgumentNullException.ThrowIfNull(paths);
        ...
        return new ZaModMergerStageResult(workflow, preview, diagnostics);
    }

    public ZaModMergerApplyResult Apply(
        ProjectPaths paths,
        IReadOnlyList<ZaModMergerSourceRequest> modSources)
    {
        ArgumentNullException.ThrowIfNull(paths);
        ...
        if (!preview.CanApply)
        {
            diagnostics.Add(CreateDiagnostic(
                ...
                "ZA Mod Merger cannot apply until the source list and project paths are valid.",
        ...
        if (string.IsNullOrWhiteSpace(outputRoot))
        {
            diagnostics.Add(CreateDiagnostic(
                DiagnosticSeverity.Error,
                "ZA Mod Merger apply requires an Output Root.",
    ...
    private static MergeAnalysis Analyze(
        ...
        IReadOnlyList<ZaModMergerSourceRequest> requests,
        ICollection<ValidationDiagnostic> diagnostics)
    {
        ...
        if (!project.Health.CanOpenEditableWorkflows)
        {
            diagnostics.Add(CreateDiagnostic(
                DiagnosticSeverity.Error,
                "ZA Mod Merger requires valid base paths and a valid Output Root.",
                expected: "Editable Pokemon Legends ZA project paths"));
        }
        ...
        if (requests.Count == 0)
        {
            diagnostics.Add(CreateDiagnostic(
                ...
                "Add at least one folder, zip, or rar mod source before staging ZA Mod Merger.",
        ...
        if (enabledSourceCount > 0 && outputs.Count == 0)
        {
            diagnostics.Add(CreateDiagnostic(
                DiagnosticSeverity.Warning,
                "No RomFS files were found in the enabled ZA mod sources.",
    ...
    private static MergePlan CreatePlan(
        ...
        ICollection<ValidationDiagnostic> diagnostics)
    {
        var files = new List<ZaModMergerFilePreviewRecord>();
    ...
    private static ZaModMergerFilePreviewRecord CreatePreviewRecord(
        OutputFileState file,
        SourceOccurrenceState winningSource,
        string mergeKind,
        string summary)
    {
        return new ZaModMergerFilePreviewRecord(
            file.RelativePath,
            file.RelativePath,
            "Pokemon Legends ZA RomFS file",
            "ready",
            mergeKind,
            summary,
    ...
    private static ZaModMergerFilePreviewRecord CreateErrorPreview(
        OutputFileState file,
        string summary)
    {
    ...
    private static ZaModMergerPreview CreatePreview(
        OpenedProject project,
        ...
        ICollection<ValidationDiagnostic> diagnostics)
    {
        ...
        foreach (var entry in manifest.Entries)
        {
            ...
            if (!string.Equals(actualHash, entry.Sha256, StringComparison.OrdinalIgnoreCase))
            {
                diagnostics.Add(CreateDiagnostic(
                    DiagnosticSeverity.Warning,
                    "A previous ZA Mod Merger output was changed outside the merger, so KM left it in place.",
        ...
        ICollection<ValidationDiagnostic> diagnostics)
    {
        ...
        try
        {
            return JsonSerializer.Deserialize<ZaModMergerManifest>(
    ...
    private sealed record SmartMergeResult(
        byte[] Contents,
        ZaModMergerFilePreviewRecord Preview);
    ...
    private sealed record ZaModMergerManifest(IReadOnlyList<ZaModMergerManifestEntry> Entries);

    private sealed record ZaModMergerManifestEntry(string RelativePath, string Sha256);
    ...
    private sealed class SourceAnalysisState
    {
        ...
        public ZaModMergerSourceRecord ToRecord()
        {
            return new ZaModMergerSourceRecord(
                SourceIndex,
                Path,
                Name,
                Kind,
    ...
    private sealed class BaseFileReader : IDisposable
    {
            ...
            ICollection<ValidationDiagnostic> diagnostics)
        {
            if (string.IsNullOrWhiteSpace(paths.BaseRomFsPath))
            {
                diagnostics.Add(CreateDiagnostic(
                    DiagnosticSeverity.Warning,
                    "ZA smart merge cannot read vanilla files until Base RomFS is set.",
                    field: "baseRomFsPath",
                    expected: "Pokemon Legends ZA Base RomFS"));
                return null;
            }

            if (!ZaCompressionRuntime.IsConfigured(paths.PokemonLegendsZASupportFolderPath))
            {
                diagnostics.Add(CreateDiagnostic(
                    DiagnosticSeverity.Warning,
                    "Pokemon Legends ZA support folder is not configured. Smart merge can still use loose base files, but packed vanilla comparisons may fall back to source priority.",
                    field: "pokemonLegendsZASupportFolderPath",
                    expected: "Configured Pokemon Legends ZA support folder for packed vanilla comparisons"));
            }

            return new BaseFileReader(paths.BaseRomFsPath, paths.PokemonLegendsZASupportFolderPath);
        }
        ...
        public bool TryRead(string relativePath, out byte[] bytes)
        {
            ...
            try
            {
                archive ??= ZaTrinityArchive.Open(baseRomFsPath, supportFolderPath);
