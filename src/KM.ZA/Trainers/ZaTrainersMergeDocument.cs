...
public static class ZaTrainersMergeDocument
{
    public static MergeDocument Read(byte[] bytes) => ZaTrainersEditSessionService.ReadMergeDocument(bytes);
}
...
internal sealed partial class ZaTrainersEditSessionService
{
    ...
    {
        var table = KM.Formats.ZA.Generated.GameData.ZaTrainerTable.GetRootAsZaTrainerTable(new ByteBuffer(bytes));
