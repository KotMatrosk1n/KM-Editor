...
public sealed class SwShModelTextureEditSessionService
{
    public EditSession Stage(ProjectPaths paths, EditSession? session, ModelTextureIntent intent)
    {
        ...
        var resource = new SwShModelPreviewService().Textures(project, intent.Model).SingleOrDefault(t => t.Id == intent.Texture)
        ...
        if (encoded.ChangedPixels > 0)
        {
            ...
            remaining.Add(new(ModelTextureIntent.Domain, $"Recolor {Path.GetFileName(intent.Texture)}", [new(ProjectFileLayer.Base, "romfs/" + target)],
    ...
    public ChangePlan CreateChangePlan(ProjectPaths paths, EditSession session)
    {
        try
        {
            ...
            var writes = outputs.Select(pair => new PlannedFileWrite(pair.Key, [new(ProjectFileLayer.Base, pair.Key)],
                File.Exists(Path.Combine(paths.OutputRootPath!, pair.Key)), "Apply verified texture colors.",
        ...
        try
        {
            ...
            foreach (var (path, bytes) in outputs)
            {
                var target = SwShOutputRollbackScope.ResolvePhysicalContainedPath(verified.ApplyPaths.OutputRootPath, path)
                    ?? throw new InvalidDataException("Texture output target is invalid.");
                Directory.CreateDirectory(Path.GetDirectoryName(target)!);
                File.WriteAllBytes(target, bytes);
                if (!File.ReadAllBytes(target).AsSpan().SequenceEqual(bytes)) throw new IOException("Texture output verification failed.");
    ...
    private static Dictionary<string, byte[]> Outputs(ProjectPaths paths, EditSession session)
    {
        ...
        foreach (var edit in session.PendingEdits)
        {
            ...
            if (intent.Game != paths.SelectedGame.ToString() || edit.RecordId != intent.Texture || intent.EncodedHash == "" || !seen.Add(intent.Texture))
                throw new InvalidDataException("Texture edit binding is invalid or duplicated.");
            if (!resources.TryGetValue(intent.Model, out var models)) resources.Add(intent.Model, models = new SwShModelPreviewService().Textures(project, intent.Model));
            ...
            if (result.ChangedPixels == 0) throw new InvalidDataException("Texture edit has no changes.");
            var target = Target(paths, resource);
        ...
        foreach (var (path, pack) in packs)
        {
            ...
            foreach (var edit in session.PendingEdits)
            {
                ...
                if (Target(paths, resource) == path && ModelTextureIntent.Hash(readback.GetFileByName(Path.GetFileName(resource.Id))) != intent.EncodedHash)
                    throw new InvalidDataException("Packed texture verification failed.");
            }
            outputs.Add("romfs/" + path, bytes);
        }
        source.Verify();
        if (outputs.Values.Sum(b => (long)b.Length) > 128L * 1024 * 1024) throw new InvalidDataException("Texture output exceeds the editing budget.");
        return outputs;
    }
    private static string Target(ProjectPaths paths, ModelTextureResource resource)
    {
        ...
        return resource.Archive ?? throw new InvalidDataException("Texture archive is missing.");
    }
