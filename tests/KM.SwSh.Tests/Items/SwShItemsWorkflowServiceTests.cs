...
public sealed class SwShItemsWorkflowServiceTests
{
    ...
    private sealed class TemporarySwShProject : IDisposable
    {
        ...
        {
            var rootPath = Path.Combine(Path.GetTempPath(), "km-editor-swsh-items-tests", Guid.NewGuid().ToString("N"));
            Directory.CreateDirectory(rootPath);

            return new TemporarySwShProject(rootPath);
        }
